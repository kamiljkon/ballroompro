...
def new_partnership(useremail):
    ...
    with open("userdata.json", "r") as f:
        ...
        for email in f_data:
            if partnername in f_data[email]["name"]:
                potentialpartners.append((f_data[email]["name"], f_data[email]["email"], f_data[email]["userid"]))
        ...
        if len(potentialpartners) > 1:
            ...
            for partner in potentialpartners:
                print(f"Partner {i}:\n Name: {partner[0]}\n E-mail: {partner[1]}")
                print(f"User-id: {partner[2]}\n")
                i +=1
            partnerchoice = getinput("xchoices", "Found multiple partners, please select the correct one from the list: ", len(potentialpartners))
            partnerid = potentialpartners[partnerchoice-1][2]
